...
const HERO_ID := "B10_MERFOLK_MUKWAREE"
...
func _check_profile(failures: Array[String]) -> void:
	...
	if String(profile.get("signature", "")) != "blossom":
		failures.append("B10 should use blossom support signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("B10 support skill should not use the generic projectile streak.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("B10 should have personal pearl splash wave VFX.")

func _check_mapping(failures: Array[String]) -> void:
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("B10 should use readable front-diagonal support rows, not side-locked melee rows.")
...
func _check_ranged_support_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("B10 controlled support duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("B10 support attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("B10 support attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("B10 walked backward during controlled approach in %d frames." % backward_walk_frames)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 120.0:
			failures.append("B10 support moved too far before/while attacking: %s %.1f px" % [unit_id, forward_delta])
