...
const HERO_ID := "D07_GARUDA_LOMPEEK"
...
func _check_profile(failures: Array[String]) -> void:
	...
	battle.free()
	if String(profile.get("personal_vfx", "")) != "skill_personal_lompeek_wind_feather_dart_v102":
		failures.append("D07 should keep wind feather dart personal VFX.")
	if runtime_mode != "":
		failures.append("D07 should use the generic dart projectile flow, got %s." % runtime_mode)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D07 should use side-locked combat rows like the other Garuda projectile heroes.")
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("D07 controlled ranged duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("D07 ranger attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("D07 attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("D07 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("D07 facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		var forward_delta: float = (float(last_x[unit_id]) - float(first_x[unit_id])) * (1.0 if unit_id == "p1" else -1.0)
		if forward_delta > 70.0:
			failures.append("D07 ranger moved too far before attacking: %s %.1f px" % [unit_id, forward_delta])
