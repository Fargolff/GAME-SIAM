...
const HERO_ID := "C05_VANARA_JORJAN"
...
func _check_profile(failures: Array[String]) -> void:
	...
	battle.free()
	if String(profile.get("signature", "")) != "slash":
		failures.append("C05 should use slash signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("C05 melee cleave should not use generic projectile streak.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("C05 should have personal monkey blade cleave VFX.")
	if mode != "melee_cleave":
		failures.append("C05 personal skill VFX mode should be melee_cleave, got %s." % mode)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("C05 should use side-locked combat rows for melee readability.")
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("C05 controlled melee duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("C05 melee attack exceeds reach tolerance in %d frames." % overreach_frames)
	if too_far_attack_frames > 0:
		failures.append("C05 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if backward_walk_frames > 0:
		failures.append("C05 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("C05 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("C05 melee unit did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
