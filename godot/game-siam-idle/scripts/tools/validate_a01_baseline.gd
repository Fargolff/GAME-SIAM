...
const HERO_ID := "A01_KINNARI_PIMPPRUEKSA"
...
func _initialize() -> void:
	...
	_check_controlled_support_runtime(failures)
	if not failures.is_empty():
		for failure in failures:
			push_error(failure)
		quit(1)
		return
	print("validate_a01_baseline PASS")
	quit(0)
...
func _check_controlled_support_runtime(failures: Array[String]) -> void:
	...
	var attack_range: float = float(base.get("range", 140.0))
	var cooldown: float = float(base.get("cooldown", 1.55))
	var move_speed: float = float(base.get("move_speed", 70.0))
	sim.units = [
	...
	var walk_frames := 0
	var wrong_facing := ""
	var first_attack_distance := 0.0
	var max_lane_drift := 0.0
	...
	for _i in range(260):
		...
		for unit in sim.units:
			...
			var position := unit["position"] as Vector2
			var current_x: float = position.x
			max_lane_drift = maxf(max_lane_drift, absf(position.y - 365.0))
			if String(unit["state"]) == "walk":
				walk_frames += 1
				if (current_x - float(previous_x[unit_id])) * forward_sign < -0.05:
			...
			if by_id.has(target_id):
				...
				var distance: float = position.distance_to(target["position"] as Vector2)
				if first_attack_distance <= 0.0:
					first_attack_distance = distance
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("A01 controlled support duel did not produce attack frames.")
	if first_attack_distance < 560.0:
		failures.append("A01 support should open from long range, got %.1f px." % first_attack_distance)
	if walk_frames > 140:
		failures.append("A01 support duel walked too much before firing: %d combined frames." % walk_frames)
	if overreach_frames > 0:
		failures.append("A01 support attacked beyond readable reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("A01 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if max_lane_drift > 12.0:
		failures.append("A01 drifted off lane by %.1f px." % max_lane_drift)
	if wrong_facing != "":
		failures.append("A01 support facing should stay readable east/west, got %s." % wrong_facing)
