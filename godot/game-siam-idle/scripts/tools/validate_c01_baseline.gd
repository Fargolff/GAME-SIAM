...
const HERO_ID := "C01_HUMAN_JETSIAM"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_c01_baseline PASS")
	quit(0)

func _check_profile(failures: Array[String]) -> void:
	if HeroCatalogScript.archetype_for(HERO_ID) != "vanguard":
		failures.append("C01 should remain vanguard.")
	if HeroCatalogScript.skill_template_for(HERO_ID) != "single_target_burst":
		failures.append("C01 should remain single_target_burst.")
	var battle := BattleSceneScript.new()
	var profile: Dictionary = battle._skill_profile(HERO_ID)
	var mode := battle._personal_vfx_runtime_mode(profile)
	battle.free()
	if String(profile.get("signature", "")) != "blow":
		failures.append("C01 should read as close-range strike/blow, got %s." % String(profile.get("signature", "")))
	if bool(profile.get("show_streak", true)):
		failures.append("C01 close melee skill should not use generic ranged streak.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("C01 should have personal krabi thrust VFX.")
	if mode != "melee_thrust":
		failures.append("C01 personal VFX runtime mode should be melee_thrust, got %s." % mode)
	...
	if walk_scale > 0.59:
		failures.append("C01 walk playback should be slowed for readable foot motion, got %.2f." % walk_scale)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("C01 should use side-locked melee combat rows.")
	if not UnitViewScript.SIDE_LOCKED_WALK_IDS.has(HERO_ID):
		failures.append("C01 should use side-locked walk rows in combat approach.")
	for walk_direction in ["north-east", "south-east"]:
		_expect_direction(failures, "walk", walk_direction, "east")
	for walk_direction in ["north-west", "south-west"]:
		_expect_direction(failures, "walk", walk_direction, "west")
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("C01 controlled melee duel did not produce attack frames.")
	if too_far_attack_frames > 0:
		failures.append("C01 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if overreach_frames > 0:
		failures.append("C01 melee attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("C01 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("C01 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("C01 melee unit did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
