...
const HERO_ID := "D09_CONSTRUCT_SILADIN"
const CURATED_SEQUENCE := [1, 2, 3, 3, 2, 1]

func _initialize() -> void:
	var failures: Array[String] = []
	_check_profile(failures)
	_check_mapping_and_frames(failures)
	_check_summoner_runtime(failures)
	if not failures.is_empty():
		for failure in failures:
			push_error(failure)
		quit(1)
		return
	print("validate_d09_baseline PASS")
	quit(0)

func _check_profile(failures: Array[String]) -> void:
	if HeroCatalogScript.archetype_for(HERO_ID) != "summoner":
		failures.append("D09 should remain summoner.")
	if HeroCatalogScript.skill_template_for(HERO_ID) != "summon_lite_visual":
		failures.append("D09 should remain summon_lite_visual.")
	...
	battle.free()
	if String(profile.get("personal_vfx", "")) != "skill_personal_siladin_stone_fist_ground_slam_v98":
		failures.append("D09 should keep Siladin stone fist ground slam personal VFX.")
	if String(profile.get("signature", "")) != "summon":
		failures.append("D09 should use summon skill signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("D09 summon should not use straight debug-like streak VFX.")
	if runtime_mode != "":
		failures.append("D09 should use generic summon VFX flow, got custom mode %s." % runtime_mode)

func _check_mapping_and_frames(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D09 should use side-locked combat rows to avoid back-readable diagonal construct frames.")
	for action in ["attack_01", "skill_01"]:
		...
		for direction in ["east", "west"]:
			...
			if sequence != CURATED_SEQUENCE:
				failures.append("%s %s sequence should be %s, got %s" % [action, direction, CURATED_SEQUENCE, sequence])
...
func _check_summoner_runtime(failures: Array[String]) -> void:
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("D09 controlled ranged duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("D09 summoner attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("D09 attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("D09 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("D09 facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 110.0:
			failures.append("D09 summoner moved too far before attacking: %s %.1f px" % [unit_id, forward_delta])
