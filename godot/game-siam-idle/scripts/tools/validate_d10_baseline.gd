...
const HERO_ID := "D10_SPIRIT_OUNRUEN"
...
func _check_profile(failures: Array[String]) -> void:
	...
	battle.free()
	if String(profile.get("personal_vfx", "")) != "skill_personal_ounruen_wispy_dash_strike_v98":
		failures.append("D10 should keep Ounruen wispy dash personal VFX.")
	if String(profile.get("signature", "")) != "blossom":
		failures.append("D10 should use blossom/support skill signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("D10 support skill should not use straight debug-like streak VFX.")
	if runtime_mode != "":
		failures.append("D10 should use generic support VFX flow, got custom mode %s." % runtime_mode)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D10 should use side-locked combat rows because east/west rows are the clean readable rows.")
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("D10 controlled ranged duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("D10 support attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("D10 attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("D10 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("D10 facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		var forward_delta: float = (float(last_x[unit_id]) - float(first_x[unit_id])) * (1.0 if unit_id == "p1" else -1.0)
		if forward_delta > 105.0:
			failures.append("D10 support moved too far before attacking: %s %.1f px" % [unit_id, forward_delta])
