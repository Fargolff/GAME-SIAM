...
const HERO_ID := "B06_BEAST_SINGKHON"
...
func _check_profile(failures: Array[String]) -> void:
	...
	if String(profile.get("signature", "")) != "guard":
		failures.append("B06 should use guard/taunt signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("B06 taunt should not use generic ranged streak.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("B06 should have personal fang/roar VFX.")
	...
	if walk_scale < 0.76:
		failures.append("B06 walk animation playback should stay readable at guardian speed, got %.2f." % walk_scale)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("B06 should use side-locked combat rows for beast melee readability.")
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("B06 controlled melee duel did not produce attack frames.")
	if too_far_attack_frames > 0:
		failures.append("B06 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if overreach_frames > 0:
		failures.append("B06 melee attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("B06 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("B06 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("B06 guardian did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
