...
const HERO_ID := "B09_SPIRIT_AMBERNIGHT"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_b09_baseline PASS")
	quit(0)

func _check_profile(failures: Array[String]) -> void:
	if HeroCatalogScript.archetype_for(HERO_ID) != "control_mage":
		failures.append("B09 should remain control_mage.")
	if HeroCatalogScript.skill_template_for(HERO_ID) != "aoe_pulse":
		failures.append("B09 should remain aoe_pulse.")
	...
	battle.free()
	if String(profile.get("signature", "")) != "pulse":
		failures.append("B09 should use pulse signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("B09 pulse should use personal amber travel, not generic streak.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("B09 should have personal amber pulse VFX.")
	if runtime_mode != "pulse_hold":
		failures.append("B09 should use pulse_hold runtime VFX.")

func _check_mapping(failures: Array[String]) -> void:
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("B09 should keep caster diagonal combat rows instead of melee side-lock.")
	if UnitViewScript.combat_readable_direction_for(HERO_ID, "attack_01", "east") != "south-east":
		failures.append("B09 east attack should use readable south-east caster row.")
	if UnitViewScript.combat_readable_direction_for(HERO_ID, "skill_01", "west") != "south-west":
		failures.append("B09 west skill should use readable south-west caster row.")
...
func _check_ranged_runtime(failures: Array[String]) -> void:
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("B09 controlled ranged duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("B09 control mage attacked too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("B09 ranged attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("B09 walked backward during controlled ranged approach in %d frames." % backward_walk_frames)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 95.0:
			failures.append("B09 control mage walked too far before attacking: %s %.1f px" % [unit_id, forward_delta])
