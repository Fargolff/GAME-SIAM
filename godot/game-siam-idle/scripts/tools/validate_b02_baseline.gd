...
const HERO_ID := "B02_NAGA_KLEDKRAM"
const REQUIRED_ANIMATIONS := [
	"walk_east",
	"walk_west",
	"attack_01_south-east",
	"attack_01_south-west",
	"skill_01_south-east",
	"skill_01_south-west",
]
...
func _initialize() -> void:
	...
	if not UnitViewScript.SLITHER_WALK_IDS.has(HERO_ID):
		failures.append("B02 should use slither walk smoothing.")
	if not failures.is_empty():
		for failure in failures:
			push_error(failure)
		quit(1)
		return
	print("validate_b02_baseline PASS")
	quit(0)
...
func _check_direction_mapping(failures: Array[String]) -> void:
	for action in ["attack_01", "skill_01"]:
		...
		_expect_direction(failures, action, "south-east", "south-east")
		_expect_direction(failures, action, "west", "south-west")
		_expect_direction(failures, action, "north-west", "south-west")
		_expect_direction(failures, action, "south-west", "south-west")
...
func _check_sprite_frames(failures: Array[String]) -> void:
	...
	if frames.get_animation_loop("attack_01_south-east"):
		failures.append("attack_01_south-east should not loop")
	if frames.get_animation_loop("skill_01_south-east"):
		failures.append("skill_01_south-east should not loop")
...
func _check_ranged_runtime(failures: Array[String]) -> void:
	...
	var attack_range: float = float(base.get("range", 150.0))
	var cooldown: float = float(base.get("cooldown", 1.65))
	var move_speed: float = float(base.get("move_speed", 68.0))
	sim.units = [
	...
	for _i in range(260):
		...
		for unit in sim.units:
			...
			if shown_direction != "south-east" and shown_direction != "south-west":
	...
	if attack_frames <= 0:
		failures.append("B02 controlled ranged duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("B02 ranged attack exceeds reach in %d frames." % overreach_frames)
	if close_attack_frames > 0:
		failures.append("B02 ranged attack happened too close in %d frames." % close_attack_frames)
	if backward_walk_frames > 0:
		failures.append("B02 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("B02 ranged facing should stay readable front-diagonal, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 120.0:
			failures.append("B02 ranged unit moved too far before/while attacking: %s %.1f px" % [unit_id, forward_delta])
