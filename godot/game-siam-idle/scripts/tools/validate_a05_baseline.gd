...
const HERO_ID := "A05_MAKARA_MAKORNKRAM"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_a05_baseline PASS")
	quit(0)
...
func _check_controlled_melee_runtime(failures: Array[String]) -> void:
	...
	var move_speed: float = float(base.get("move_speed", 78.0))
	sim.units = [
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 2.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("A05 controlled melee duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("A05 melee attack reads too far in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("A05 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("A05 melee facing should stay readable east/west, got %s." % wrong_facing)
