...
func _check_profile(failures: Array[String]) -> void:
	...
	if String(profile.get("signature", "")) != "shot":
		failures.append("D03 should use bow shot signature, got %s." % String(profile.get("signature", "")))
	if not bool(profile.get("show_streak", false)):
		failures.append("D03 ranged shot should keep projectile streak readable.")
	if runtime_mode != "bow_release":
		failures.append("D03 should use bow_release runtime mode, got %s." % runtime_mode)
	if String(profile.get("personal_vfx", "")) != "skill_personal_prannoi_wood_bow_shot_v103":
		failures.append("D03 should keep its personal wood bow shot VFX.")

func _check_mapping(failures: Array[String]) -> void:
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D03 should use readable diagonal ranged rows, not side-locked melee rows.")
...
func _check_ranger_runtime(failures: Array[String]) -> void:
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("D03 controlled ranger duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("D03 ranger attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("D03 ranger attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("D03 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("D03 ranger facing should stay readable diagonal, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 95.0:
			failures.append("D03 ranger moved too far before/while attacking: %s %.1f px" % [unit_id, forward_delta])
