...
const HERO_ID := "C10_SPIRIT_KHOMKHAM"

func _initialize() -> void:
	var failures: Array[String] = []
	_check_profile(failures)
	_check_mapping(failures)
	_check_ranged_debuffer_runtime(failures)
	if not failures.is_empty():
		for failure in failures:
			push_error(failure)
		quit(1)
		return
	print("validate_c10_baseline PASS")
	quit(0)

func _check_profile(failures: Array[String]) -> void:
	if HeroCatalogScript.archetype_for(HERO_ID) != "debuffer":
		failures.append("C10 should remain debuffer.")
	if HeroCatalogScript.skill_template_for(HERO_ID) != "debuff_attack":
		failures.append("C10 should remain debuff_attack.")
	var battle := BattleSceneScript.new()
	var profile: Dictionary = battle._skill_profile(HERO_ID)
	battle.free()
	if String(profile.get("signature", "")) != "hex":
		failures.append("C10 should use hex/debuff signature.")
	if String(profile.get("personal_vfx", "")) != "skill_personal_khomkham_soul_flame_seal_v99":
		failures.append("C10 should keep its personal soul flame seal VFX.")

func _check_mapping(failures: Array[String]) -> void:
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("C10 should keep readable diagonal spirit-caster combat rows, not melee side-lock.")
...
func _check_ranged_debuffer_runtime(failures: Array[String]) -> void:
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("C10 controlled debuffer duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("C10 debuffer attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("C10 debuffer attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("C10 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("C10 debuffer facing should stay readable diagonal, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		var forward_delta: float = (float(last_x[unit_id]) - float(first_x[unit_id])) * (1.0 if unit_id == "p1" else -1.0)
		if forward_delta > 115.0:
			failures.append("C10 debuffer moved too far before/while attacking: %s %.1f px" % [unit_id, forward_delta])
