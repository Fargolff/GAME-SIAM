...
const HERO_ID := "A03_HUMAN_ARUNRAT"
...
func _initialize() -> void:
	...
	_check_controlled_ranged_runtime(failures)
	if not failures.is_empty():
		for failure in failures:
			push_error(failure)
		quit(1)
		return
	print("validate_a03_baseline PASS")
	quit(0)
...
func _check_controlled_ranged_runtime(failures: Array[String]) -> void:
	...
	var attack_range: float = float(base.get("range", 165.0))
	var cooldown: float = float(base.get("cooldown", 1.4))
		...
		for unit in sim.units:
			...
			if first_attack_distance <= 0.0:
				...
				if by_id.has(target_id):
					...
					first_attack_distance = (unit["position"] as Vector2).distance_to(target["position"] as Vector2)
	if attack_frames <= 0:
		failures.append("A03 controlled ranged duel did not produce attack frames.")
	if first_attack_distance < 300.0:
		failures.append("A03 should open fire from long range, got %.1f px." % first_attack_distance)
	if walk_frames > 120:
		failures.append("A03 ranged duel walked too much before firing: %d combined frames." % walk_frames)
	if backward_walk_frames > 0:
		failures.append("A03 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("A03 ranged facing should stay readable east/west, got %s." % wrong_facing)
