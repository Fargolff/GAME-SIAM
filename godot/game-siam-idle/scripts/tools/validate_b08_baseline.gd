...
const HERO_ID := "B08_HUMAN_MUENMONTRA"
...
func _check_profile(failures: Array[String]) -> void:
	...
	battle.free()
	if String(profile.get("signature", "")) != "hex":
		failures.append("B08 should use hex signature.")
	if not bool(profile.get("show_streak", false)):
		failures.append("B08 hex should keep a readable ranged streak/path.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("B08 should have personal mantra paper-tear VFX.")
	if runtime_mode != "paper_hex":
		failures.append("B08 should use paper_hex runtime VFX for readable ranged hex travel.")

func _check_mapping(failures: Array[String]) -> void:
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("B08 should keep caster diagonal combat rows instead of melee side-lock.")
	if UnitViewScript.combat_readable_direction_for(HERO_ID, "attack_01", "east") != "south-east":
		failures.append("B08 east attack should use readable south-east caster row.")
	if UnitViewScript.combat_readable_direction_for(HERO_ID, "skill_01", "west") != "south-west":
		failures.append("B08 west skill should use readable south-west caster row.")
...
func _check_ranged_runtime(failures: Array[String]) -> void:
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("B08 controlled ranged duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("B08 debuffer attacked too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("B08 ranged attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("B08 walked backward during controlled ranged approach in %d frames." % backward_walk_frames)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 110.0:
			failures.append("B08 debuffer walked too far before attacking: %s %.1f px" % [unit_id, forward_delta])
