...
const HERO_ID := "D05_VANARA_JUKJIK"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_d05_baseline PASS")
	quit(0)

func _check_profile(failures: Array[String]) -> void:
	if HeroCatalogScript.archetype_for(HERO_ID) != "skirmisher":
		failures.append("D05 should remain skirmisher.")
	if HeroCatalogScript.skill_template_for(HERO_ID) != "cleave":
		failures.append("D05 should remain cleave.")
	...
	battle.free()
	if String(profile.get("personal_vfx", "")) != "skill_personal_jukjik_twin_claw_cut_v102":
		failures.append("D05 should keep its twin claw personal VFX.")
	if runtime_mode != "melee_cleave":
		failures.append("D05 should use melee_cleave VFX flow, got %s." % runtime_mode)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D05 should use side-locked combat rows to avoid back-readable diagonal melee frames.")
	...
	var tuned_speed := UnitViewScript.walk_speed_scale_for_character(HERO_ID, 132.0)
	var default_speed := UnitViewScript.walk_speed_scale_for_character("", 132.0)
	if tuned_speed >= default_speed:
		failures.append("D05 walk animation should be slightly slower than default at reference speed.")
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
				if distance > 72.0:
					too_far_melee_frames += 1
	if attack_frames <= 0:
		failures.append("D05 controlled melee duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("D05 attack exceeds reach in %d frames." % overreach_frames)
	if too_far_melee_frames > 0:
		failures.append("D05 melee attack still reads too far in %d frames." % too_far_melee_frames)
	if backward_walk_frames > 0:
		failures.append("D05 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("D05 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		var forward_delta: float = (float(last_x[unit_id]) - float(first_x[unit_id])) * (1.0 if unit_id == "p1" else -1.0)
		if forward_delta < 245.0 or forward_delta > 355.0:
			failures.append("D05 melee approach distance should stay natural, got %s %.1f px" % [unit_id, forward_delta])
