...
const HERO_ID := "D06_NAGA_BUABUCHA"

func _initialize() -> void:
	var failures: Array[String] = []
	_check_profile(failures)
	_check_animation_mapping(failures)
...
func _check_profile(failures: Array[String]) -> void:
	...
	battle.free()
	if String(profile.get("personal_vfx", "")) != "skill_personal_buabucha_lotus_root_bind_v98":
		failures.append("D06 should keep lotus root bind personal VFX.")
	if runtime_mode != "target_seal":
		failures.append("D06 root bind should use target_seal VFX flow, got %s." % runtime_mode)

func _check_animation_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SLITHER_WALK_IDS.has(HERO_ID):
		failures.append("D06 should use slither walk tuning as a naga.")
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D06 should preserve caster diagonals instead of side-locking every cast.")
	for action in ["attack_01", "skill_01"]:
		_expect_direction(failures, action, "east", "south-east")
		_expect_direction(failures, action, "north-east", "south-east")
		_expect_direction(failures, action, "west", "south-west")
		_expect_direction(failures, action, "north-west", "south-west")
...
func _check_ranged_runtime(failures: Array[String]) -> void:
	...
	for _i in range(260):
		...
		for unit in sim.units:
			...
			if shown_direction in ["north", "north-east", "north-west"]:
				wrong_back_facing = shown_direction
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("D06 controlled ranged duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("D06 control mage attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("D06 attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("D06 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_back_facing != "":
		failures.append("D06 should not show back-facing combat rows in horizontal duel, got %s." % wrong_back_facing)
	for unit_id in ["p1", "e1"]:
		var forward_delta: float = (float(last_x[unit_id]) - float(first_x[unit_id])) * (1.0 if unit_id == "p1" else -1.0)
		if forward_delta > 85.0:
			failures.append("D06 ranged/control mage moved too far before attacking: %s %.1f px" % [unit_id, forward_delta])
