...
const HERO_ID := "B03_YAKSHA_KHUNPHA"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_b03_baseline PASS")
	quit(0)

func _check_direction_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("B03 should use side-locked combat rows for melee cleave readability.")
...
func _check_skill_vfx_mode(failures: Array[String]) -> void:
	...
	if mode != "melee_cleave":
		failures.append("B03 personal skill VFX mode should be melee_cleave, got %s." % mode)
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	var attack_range: float = float(base.get("range", 46.0))
	var cooldown: float = float(base.get("cooldown", 1.25))
	var move_speed: float = float(base.get("move_speed", 86.0))
	sim.units = [
	...
	if attack_frames <= 0:
		failures.append("B03 controlled melee duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("B03 melee attack exceeds reach tolerance in %d frames." % overreach_frames)
	if too_far_attack_frames > 0:
		failures.append("B03 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if backward_walk_frames > 0:
		failures.append("B03 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("B03 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("B03 melee unit did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
