...
const HERO_ID := "C08_CROCODILE_KUMPHIL"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_c08_baseline PASS")
	quit(0)

func _check_profile(failures: Array[String]) -> void:
	if HeroCatalogScript.archetype_for(HERO_ID) != "guardian":
		failures.append("C08 should remain guardian.")
	if HeroCatalogScript.skill_template_for(HERO_ID) != "taunt":
		failures.append("C08 should remain taunt.")
	var battle := BattleSceneScript.new()
	var profile: Dictionary = battle._skill_profile(HERO_ID)
	battle.free()
	if String(profile.get("signature", "")) != "aegis":
		failures.append("C08 should use crocodile aegis signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("C08 taunt/aegis should not use generic ranged streak.")
	if String(profile.get("personal_vfx", "")) != "skill_personal_kumphil_crocodile_aegis_v102":
		failures.append("C08 should keep its personal crocodile aegis VFX.")
	...
	if walk_scale > 0.64:
		failures.append("C08 heavy crocodile walk playback should stay weighty, got %.2f." % walk_scale)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("C08 should use side-locked combat rows for crocodile melee readability.")
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("C08 controlled melee duel did not produce attack frames.")
	if too_far_attack_frames > 0:
		failures.append("C08 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if overreach_frames > 0:
		failures.append("C08 melee attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("C08 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("C08 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("C08 guardian did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
