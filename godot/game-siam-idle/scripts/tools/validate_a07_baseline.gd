...
const HERO_ID := "A07_HUMAN_CHANGJAKKAEW"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_a07_baseline PASS")
	quit(0)
...
func _check_ranged_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("A07 controlled ranged duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("A07 ranged attack exceeds reach in %d frames." % overreach_frames)
	if close_attack_frames > 0:
		failures.append("A07 ranged attack happened too close in %d frames." % close_attack_frames)
	if backward_walk_frames > 0:
		failures.append("A07 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("A07 ranged facing should stay readable east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 120.0:
			failures.append("A07 ranged unit moved too far before/while attacking: %s %.1f px" % [unit_id, forward_delta])
