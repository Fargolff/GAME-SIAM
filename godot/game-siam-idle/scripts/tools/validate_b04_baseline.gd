...
func _initialize() -> void:
	...
	_check_skill_vfx_mode(failures)
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_b04_baseline PASS")
	quit(0)

func _check_direction_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("B04 should use side-locked combat rows so ranged attacks do not read as back-facing diagonal slashes.")
...
func _check_skill_vfx_mode(failures: Array[String]) -> void:
	var battle := BattleSceneScript.new()
	var mode := battle._personal_vfx_runtime_mode({"hero_id": HERO_ID})
	var profile: Dictionary = battle._skill_profile(HERO_ID)
	battle.free()
	if HeroCatalogScript.archetype_for(HERO_ID) != "skirmisher":
		failures.append("B04 should be skirmisher because the approved sprite reads as a dual-blade melee unit.")
	if mode != "melee_seal":
		failures.append("B04 personal skill VFX mode should be melee_seal, got %s." % mode)
	if String(profile.get("signature", "")) != "seal":
		failures.append("B04 skill signature should be seal, got %s." % String(profile.get("signature", "")))
	if bool(profile.get("show_streak", true)):
		failures.append("B04 should not use the generic projectile streak; it should use a close-range Darin seal cut.")
	if String(profile.get("personal_vfx", "")) == "":
		failures.append("B04 should have a personal VFX sheet.")
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	var cooldown: float = float(base.get("cooldown", 1.05))
	var move_speed: float = float(base.get("move_speed", 104.0))
	sim.units = [
	...
	if attack_frames <= 0:
		failures.append("B04 controlled melee duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("B04 melee attack exceeds reach in %d frames." % overreach_frames)
	if too_far_attack_frames > 0:
		failures.append("B04 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if backward_walk_frames > 0:
		failures.append("B04 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("B04 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("B04 melee unit did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
