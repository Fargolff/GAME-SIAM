...
const HERO_ID := "A04_VANARA_KALAVANARA"
...
func _initialize() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("validate_a04_baseline PASS")
	quit(0)
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 2.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("A04 controlled melee duel did not produce attack frames.")
	if overreach_frames > 0:
		failures.append("A04 melee attack reads too far in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("A04 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("A04 melee facing should stay readable east/west, got %s." % wrong_facing)
