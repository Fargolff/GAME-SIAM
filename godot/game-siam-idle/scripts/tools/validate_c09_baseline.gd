...
const HERO_ID := "C09_KINNARI_KAEWKANGSADAN"
...
func _check_profile(failures: Array[String]) -> void:
	...
	if String(profile.get("signature", "")) != "blossom":
		failures.append("C09 should use blossom/support signature.")
	if bool(profile.get("show_streak", true)):
		failures.append("C09 support skill should not use generic projectile streak.")
	if String(profile.get("personal_vfx", "")) != "skill_personal_kaew_crystal_chime_shards_v98":
		failures.append("C09 should keep its personal crystal chime shard VFX.")
	if runtime_mode != "":
		failures.append("C09 support blossom should use support-centered VFX flow, got runtime mode %s." % runtime_mode)

func _check_mapping(failures: Array[String]) -> void:
	if UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("C09 should use readable front-diagonal support rows, not side-locked melee rows.")
...
func _check_ranged_support_runtime(failures: Array[String]) -> void:
		...
		for unit in sim.units:
			...
			if by_id.has(target_id):
				...
				if distance > sim._attack_reach(unit) + 6.0:
					overreach_frames += 1
	if attack_frames <= 0:
		failures.append("C09 controlled support duel did not produce attack frames.")
	if close_attack_frames > 0:
		failures.append("C09 support attack happened too close in %d frames." % close_attack_frames)
	if overreach_frames > 0:
		failures.append("C09 support attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("C09 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("C09 support facing should stay readable diagonal, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta > 95.0:
			failures.append("C09 support moved too far before/while attacking: %s %.1f px" % [unit_id, forward_delta])
