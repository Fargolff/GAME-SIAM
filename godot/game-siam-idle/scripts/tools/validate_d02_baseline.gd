...
const HERO_ID := "D02_HUMAN_KHAMPAN"
...
func _check_profile(failures: Array[String]) -> void:
	...
	if String(profile.get("signature", "")) != "guard":
		failures.append("D02 should use the guardian guard signature, got %s." % String(profile.get("signature", "")))
	if bool(profile.get("show_streak", true)):
		failures.append("D02 guardian skill should not use generic ranged streak.")
	if runtime_mode != "melee_guard":
		failures.append("D02 personal VFX should use melee_guard runtime mode, got %s." % runtime_mode)
	if String(profile.get("personal_vfx", "")) != "skill_personal_khampan_bronze_pot_guard_v102":
		failures.append("D02 should keep its personal bronze pot guard VFX.")
	if float(profile.get("cast_size", 999.0)) > 190.0:
		failures.append("D02 cast VFX should stay compact, got %.1f." % float(profile.get("cast_size", 0.0)))
	if float(profile.get("impact_size", 999.0)) > 310.0:
		failures.append("D02 impact VFX should stay compact, got %.1f." % float(profile.get("impact_size", 0.0)))
	...
	if walk_scale > 0.66:
		failures.append("D02 walk playback should stay readable, got %.2f." % walk_scale)

func _check_mapping(failures: Array[String]) -> void:
	if not UnitViewScript.SIDE_LOCKED_ATTACK_IDS.has(HERO_ID):
		failures.append("D02 should use side-locked combat rows for melee readability.")
...
func _expect_frame_sequence(failures: Array[String], action: String, direction: String) -> void:
	...
	if sequence != [0, 2, 2, 4, 5, 0]:
		failures.append("%s %s should use curated D02 sequence [0,2,2,4,5,0], got %s." % [action, direction, str(sequence)])
	for bad_frame in [1, 3]:
		if sequence.has(bad_frame):
			failures.append("%s %s should not show back-readable frame %d." % [action, direction, bad_frame])
...
func _check_melee_runtime(failures: Array[String]) -> void:
	...
	if attack_frames <= 0:
		failures.append("D02 controlled melee duel did not produce attack frames.")
	if too_far_attack_frames > 0:
		failures.append("D02 melee attack reads too far from target in %d frames." % too_far_attack_frames)
	if overreach_frames > 0:
		failures.append("D02 melee attack exceeds reach in %d frames." % overreach_frames)
	if backward_walk_frames > 0:
		failures.append("D02 walked backward during controlled approach in %d frames." % backward_walk_frames)
	if wrong_facing != "":
		failures.append("D02 melee facing should stay side-locked east/west, got %s." % wrong_facing)
	for unit_id in ["p1", "e1"]:
		...
		if forward_delta < 220.0:
			failures.append("D02 guardian did not close enough before attacking: %s %.1f px" % [unit_id, forward_delta])
