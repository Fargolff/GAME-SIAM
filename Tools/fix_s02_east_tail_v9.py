...
FINAL_DIR = RUN_DIR / "final_v9"
QA_DIR = RUN_DIR / "qa"
BACKUP_DIR = RUN_DIR / "backup/pre_v9_east_tail_tip_fix"
...
def trim_left_tail_tip(frame: Image.Image) -> int:
    pixels = frame.load()
    changed = 0
    for y in range(48, 63):
        for x in range(0, 23):
            # Keep a little of the upper contour; only remove the pointed lower-left spur.
            if x > 17 and y < 55:
                continue
            r, g, b, a = pixels[x, y]
    ...
    outline = [(18, 49), (18, 50), (19, 51), (20, 52), (21, 53), (22, 54), (23, 55)]
    shade = [(20, 50), (21, 51), (22, 52), (23, 53), (24, 54), (25, 55)]
    highlight = [(22, 49), (23, 50), (24, 51)]
    for point in outline:
        draw.point(point, fill=(3, 19, 45, 255))
    for point in shade:
        draw.point(point, fill=(18, 76, 158, 255))
    for point in highlight:
        draw.point(point, fill=(146, 194, 230, 255))
    return changed + len(outline) + len(shade) + len(highlight)
...
def make_east_qc(out_path: Path) -> None:
    ...
    draw.text((8, 8), "S02 v9 east all frames x5", fill=(255, 255, 255), font=font)
...
def main() -> None:
    ...
    for action in ("walk", "attack_01"):
        ...
        for col in (2, 5):
            ...
            stats[f"{action}/east/f{col}"] = trim_left_tail_tip(frame)
            paste_frame(sheet, EAST_ROW, col, frame)
        sheet.save(sheet_path)
    install_final()
    east_qc = QA_DIR / "S02_v9_east_all_frames_x5.png"
    make_east_qc(east_qc)
    manifest = {
        "hero_id": "S02_NAGA_SASINAKA",
        "version": "v9_east_tail_tip_fix",
        "status": "installed_active_pending_visual_qc",
        "final_dir": str(FINAL_DIR),
        "backup_dir": str(BACKUP_DIR),
        "changes": {
            "east_tail_tip": "Trimmed the pointed lower-left spur on walk/attack_01 east f2/f5 to remove the double-tail read.",
        },
        "stats": stats,
        "qc": {"east_all_frames_x5": str(east_qc)},
    }
    (RUN_DIR / "run-manifest-v9.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
